...
- Designed for Railway deployment with persistent volume support
...
class ReportMeta:
    title: str
    mode: str  # plan | pred | final


REPORT_TYPES = {
    "План": ReportMeta("ПЛАН", "plan"),
    "Предварительный отчёт": ReportMeta("ПРЕДВАРИТЕЛЬНЫЙ ОТЧЁТ", "pred"),
    "Итоговый отчёт": ReportMeta("ИТОГОВЫЙ ОТЧЕТ", "final"),
}
...
SETTINGS_FIELDS = {
    ...
    "Плановый трафик": ("plan_traffic", "Отправь плановый трафик в формате ЧЧ:ММ:СС, например 03:00:00"),
}
...
async def settings_select(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if text == "Назад":
        await safe_reply(update, "Вернул в меню.", reply_markup=MAIN_MENU)
        return ConversationHandler.END
...
async def settings_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if field_key == "plan_traffic":
        if not re.fullmatch(r"\d{2}:\d{2}:\d{2}", text):
            await safe_reply(update, "Нужен формат ЧЧ:ММ:СС, например 03:00:00", reply_markup=CANCEL_MENU)
            return SETTINGS_INPUT
...
def valid_any_number_or_time(value: str) -> bool:
    return bool(value)
...
async def report_dozh(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if mode == "final":
        ...
        await safe_reply(
            update,
            f"Введи фактический трафик в формате ЧЧ:ММ:СС.\nПлановый трафик сейчас: {settings['plan_traffic']}",
