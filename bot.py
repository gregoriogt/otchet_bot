"""
Telegram bot for daily reports and PZM/PSM call reports.
...
SETTINGS_PROMPTS = {
    ...
    "extra_daily_lines": (
        "Введи доп. строки для ежедневных отчётов одним сообщением.\n\n"
        "Пример:\n"
        "ИТОГО: 0 / 1320.000\n"
        "% выполнения плана 0%\n\n"
        "ЦЕЛЬ: 0/ 1500.000\n"
        "% выполнения цели: 0%\n\n"
        "Ожидаемые платежи: 0 / 0\n"
        "% выполнения ожидаемых: 0 %\n\n"
        "Если доп. строки не нужны - отправь: -"
    ),
}
...
def load_settings() -> Dict[str, Dict[str, str]]:
    ...
    try:
        ...
        logger.warning("Не удалось прочитать файл настроек: %s", e)
...
def normalize_time_hms(value: str) -> Optional[str]:
...
def normalize_time_hm(value: str) -> Optional[str]:
    value = normalize_text(value)
...
def is_zero_value(value: str) -> bool:
    ...
    return cleaned in {"0", "0.0", "00", "000", "0.00", "0.000"}


def is_empty_extra_lines(value: str) -> bool:
    cleaned = normalize_multiline_text(value).lower()
    return cleaned in {"", "-", "нет", "не надо", "не нужно", "0"}


def get_extra_daily_lines(settings: Dict[str, str]) -> str:
    value = settings.get("extra_daily_lines", "")
    if is_empty_extra_lines(value):
        return ""
    return normalize_multiline_text(value)


def field_enabled_for_reports(settings: Dict[str, str], field: str) -> bool:
    setting_key = STAGE_TO_SETTING.get(field)
    if not setting_key:
        return True
    return not is_zero_value(settings.get(setting_key, "0"))


def default_value_for_stage(settings: Dict[str, str], field: str) -> str:
    setting_key = STAGE_TO_SETTING[field]
    if is_zero_value(settings.get(setting_key, "0")):
        return "0"
    return ""
...
def current_settings_field(context: ContextTypes.DEFAULT_TYPE) -> Optional[str]:
...
def build_settings_summary(user_id: int, extra: Optional[str] = None) -> str:
    s = get_user_settings(user_id)
    extra_lines = get_extra_daily_lines(s)
    ...
    lines = [
        ...
        f"План КЗ: {s['plan_kz']}",
        "",
        "Доп. строки ежедневных отчётов:",
        extra_lines if extra_lines else "не используются",
    ]
...
def start_report(context: ContextTypes.DEFAULT_TYPE, user_id: int, mode: str, title: str) -> None:
    ...
    for field, _ in STAGE_FIELDS:
        default = default_value_for_stage(settings, field)
        if default:
...
def current_step(context: ContextTypes.DEFAULT_TYPE) -> Optional[str]:
...
def build_progress_text(user_id: int, report: Dict[str, Any], prompt: str) -> str:
    ...
    for field, label in STAGE_FIELDS:
        if field in vals:
            shown = vals[field]
        elif field_enabled_for_reports(settings, field):
            shown = "—"
        else:
            shown = "0"
    ...
    lines.extend(["", "Что нужно ввести сейчас:", prompt, "", "Кнопки ниже: назад, вперёд или отмена."])
    return "\n".join(lines)
...
def build_plan_text(user_id: int) -> str:
    ...
    lines.extend(["", s["employee_hashtag"], s["city_hashtag"], s["mention"]])
    return "\n".join(lines)


def build_report_text(user_id: int, report: Dict[str, Any]) -> str:
    settings = get_user_settings(user_id)
    extra_lines = get_extra_daily_lines(settings)
    vals = report["values"]
    ...
    for field, label in STAGE_FIELDS:
        ...
        if value is None or value == "":
            value = default_value_for_stage(settings, field)
    ...
    if report["mode"] == "final":
        ...
        lines.extend([f"Трафик: {vals.get('traffic', '')}", f"КЗ: {vals.get('kz', '')}"])

    if extra_lines:
        lines.extend(["", extra_lines])

    lines.extend(["", settings["employee_hashtag"], settings["city_hashtag"], settings["mention"]])
    return "\n".join(lines)
...
CALL_REPORT_TEMPLATES = {
    ...
    "call_psm": {
        ...
        "steps": [
            ...
            ("demand", "6.1 СЛОЖИЛОСЬ ЛИ ПОНИМАНИЕ У КЛИЕНТА ПО СПРОСУ"),
            ("strengths", "6.2 СЛОЖИЛОСЬ ЛИ ПОНИМАНИЕ У КЛИЕНТА ПО СИЛЬНЫМ СТОРОНАМ БИЗНЕСА"),
            ("finance", "6.3 СЛОЖИЛОСЬ ЛИ ПОНИМАНИЕ У КЛИЕНТА ПО ФИН МОДЕЛИ"),
...
def current_call_step(context: ContextTypes.DEFAULT_TYPE) -> Optional[Tuple[str, str]]:
...
def build_call_progress_text(report: Dict[str, Any]) -> str:
    ...
    for step_key, step_label in report["steps"]:
        lines.append(f"• {step_label}: {vals.get(step_key, '—')}")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        "Привет\n\n"
        "Бот собирает ежедневные отчёты, отчёты ПЗМ и отчёты ПСМ.\n\n"
        "В ежедневных отчётах блоки с планом 0 не спрашиваются при заполнении, "
        "но в готовом отчёте всё равно выводятся как 0."
    )

    await send_prompt(update, context, text, MAIN_MENU)


async def finish_daily_report(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, report: Dict[str, Any]) -> None:
    final_text = build_report_text(user_id, report)
    await delete_previous_prompt(context)
    context.user_data.clear()
...
async def finish_call_report(update: Update, context: ContextTypes.DEFAULT_TYPE, report: Dict[str, Any]) -> None:
...
async def text_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if text == "Предварительный отчёт":
        ...
        if step is None:
            await finish_daily_report(update, context, user.id, report)
            return
    ...
    if text == "Настройки":
        ...
        if field is None:
            context.user_data.clear()
            await send_prompt(update, context, "Настройки не найдены. Попробуй /start.", MAIN_MENU)
            return
    ...
    if mode == "settings":
        ...
        if field == "extra_daily_lines":
            value = normalize_multiline_text(raw_text)
            if is_empty_extra_lines(value):
                value = ""
        else:
            value = text
    ...
    if mode == "call_report":
        ...
        if text == "➡️ Вперёд":
            ...
            if step is None:
                await finish_call_report(update, context, report)
                return
            await send_prompt(update, context, build_call_progress_text(report), NAV_MENU)
            return
        ...
        step_key, _ = step
        report["values"][step_key] = raw_text.strip()
        advance_call_step(context)
